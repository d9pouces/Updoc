...
class ProgressBar(object):
    ...
    def __init__(self, maxval=None, widgets_=None, term_width=None, poll=1,
                 left_justify=True, fd=sys.stderr):
        """Initializes a progress bar with sane defaults."""

        # Don't share a reference with any other progress bars
        if widgets_ is None:
            widgets_ = list(self._DEFAULT_WIDGETS)
        self.maxval = maxval
    ...
    def _format_line(self):
        ...
        widgets_ = ''.join(self._format_widgets())
        if self.left_justify:
            return widgets_.ljust(self.term_width)
        else:
            return widgets_.rjust(self.term_width)
